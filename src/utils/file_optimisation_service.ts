...
//   //     const media_id = Math.floor(Math.random() * 1000000000);
//   //     const thumbnail_path = `/tmp/thumbnail-${media_id}.png`;
//   //     let local_video_path = ""; // To store the path of the downloaded video

//   //     try {
//   //       // FIX: Download the video to a temporary local file first
//   //       local_video_path = await this.downloadFile(media_url);

//   //       // Now, process the local video file with ffmpeg
//   //       await new Promise<void>((resolve, reject) => {
//   //         ffmpeg(local_video_path) // Use the local file path
//   //           .outputOptions([
//   //             "-vf",
//   //             "scale=320:-1",
//   //             "-vframes",
...
//   //       if (local_video_path && fs.existsSync(local_video_path)) {
//   //         fs.unlinkSync(local_video_path);
//   //       }
...
export default class FileOptimisationService {
  ...
  async uploadFileToCloudStorage(
    ...
    try {
      const file_id = Math.floor(Math.random() * 1000000000);
      const file_key = `media/media-${file_id}`;
      await this.gcs.upload(file_path, {
        destination: file_key,
  ...
  async compressImage(media_url: string): Promise<string> {
    const media_id = Math.floor(Math.random() * 1000000000);
    const compressed_path = `/tmp/compressed-${media_id}.webp`;
    let file_path = "";
    try {
      file_path = await this.downloadFile(media_url);
      await sharp(file_path).webp({ quality: 80 }).toFile(compressed_path);
      const compressed_url = await this.uploadFileToCloudStorage(
        compressed_path,
        "image/webp"
      );
      ...
      if (fs.existsSync(compressed_path)) fs.unlinkSync(compressed_path);
    }
  }
  ...
  async handleDeletion(optimisationRecord: OptimisationRecord) {
    ...
    const deletionPromises = filesToDelete.map((filePath) =>
      this.gcs.file(filePath).delete()
    );
    try {
      await Promise.all(deletionPromises);
    } catch (error) {
      console.error("An error occurred during bulk deletion from GCP:", error);
    }
  }
}
