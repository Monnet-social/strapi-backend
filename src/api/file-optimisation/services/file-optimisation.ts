    ...
    ({ strapi }) => ({
        async optimiseFile(file_id: number) {
            try {
                ...
                const data =
                    await new FileOptimisationService().handleOptimisation(
                        file_data
                    );
                console.log(data);
        ...
        async deleteOptimisedFile(file_id: number) {
            if (!file_id || isNaN(file_id)) {
                console.warn(
                    "deleteOptimisedFile was called with an invalid file_id:",
                    file_id
                );
                return;
            }
            ...
            console.log(
                `Starting deletion process for media file id: ${file_id}`
            );
