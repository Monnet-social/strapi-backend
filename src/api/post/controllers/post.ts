...
module.exports = createCoreController("api::post.post", ({ strapi }) => ({
    async create(ctx) {
        ...
        try {
            ...
            const message = data.post_type === "post" ? "Post" : "Story";
            return ctx.send({
                post: newPost,
    ...
    async update(ctx) {
        ...
        try {
            const postToUpdate = await strapi.entityService.findOne(
                "api::post.post",
                postId,
                { populate: ["posted_by"] }
            );

            if (!postToUpdate) return ctx.notFound("Post not found");

            if (postToUpdate.posted_by.id !== userId)
    ...
    async delete(ctx) {
        ...
        try {
            const postToDelete = await strapi.entityService.findOne(
                "api::post.post",
                postId,
                { populate: ["posted_by"] }
            );

            if (!postToDelete) return ctx.notFound("Post not found");

            if (postToDelete.posted_by.id !== userId)
                return ctx.forbidden(
