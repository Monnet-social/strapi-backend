...
async function login(ctx) {
  ...
  if (!password || !email)
    return ctx.badRequest(
      "Identifier (username or email) and password must be provided."
    );

  try {
    const isEmail = HelperService.EMAIL_REGEX.test(email);
...
async function sendOTP(ctx: any) {
  try {
    ...
    if (!email || !type)
      return ctx.badRequest(
        "Identifier (username or email) and type are required."
      );
...
async function resetPassword(ctx) {
  ...
  try {
    ...
    await strapi.plugin("users-permissions").service("user").edit(payload.id, {
